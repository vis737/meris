      ...
      if (!error && data && data.length > 0) {
        const mapped = data.map((p) => {
          ...
          return {
            ...
            freeShipping: localMatch ? Boolean((p.free_shipping ?? localMatch.freeShipping) || p.id === "test-razorpay-10rs" || p.sku === "TEST-RZP-10" || p.category_slug === "test") : Boolean(p.free_shipping || p.id === "test-razorpay-10rs" || p.sku === "TEST-RZP-10" || p.category_slug === "test"),
            gstExempt: localMatch ? Boolean((p.gst_exempt ?? localMatch.gstExempt) || p.id === "test-razorpay-10rs" || p.sku === "TEST-RZP-10" || p.category_slug === "test") : Boolean(p.gst_exempt || p.id === "test-razorpay-10rs" || p.sku === "TEST-RZP-10" || p.category_slug === "test")
          };
        });
